...
class MonthCalendarWidgetState extends State<MonthCalendarWidget> {
  ...
  bool _isSelectedDate(int index) {
    ...
    if (selectedDay != null) {
      if (selectedDay.isSameOrAfter(widget.begin) && selectedDay.isSameOrBefore(widget.end)) {
        if (Jiffy.parseFromJiffy(widget.begin).add(days: index).isSame(selectedDay)) {
          return true;
        }
      }
    }
  ...
  Pair<int, bool> _getDisplayDay(int index) {
    ...
    if (consideringPrevMonth <= widget.begin.daysInMonth && widget.begin.date != 1) {
          ...
          body: widget.dayItemBuilder != null
              ? widget.dayItemBuilder?.call(
                  DayItemProperties(
                    ...
                    notFittedEventsCount: widget.overflowedEvents.weeks[row].eventCount[column],
              ...
              : DayItem(
                  ...
                  nonFitEventCount: widget.overflowedEvents.weeks[row].eventCount[column],
                ),
        ),
      );
    });
  ...
  void _performDaySelecting(Jiffy jiffyDay) {
    final events = calculateAvailableEventsForDate(_controller.events ?? [], jiffyDay);
  ...
  void _calculateRange(DateTime tappedDay) {
    ...
    if (begin == null || tappedDay.isBefore(begin) || tappedDay.isAtSameMomentAs(begin)) {
  ...
  void _useOnRangeSelectedCallback() {
    widget.onRangeSelected?.call(calculateAvailableEventsForRange(
      ...
      _controller.selectedRange.end?.toJiffy() ?? _controller.selectedRange.begin?.toJiffy(),
    ));
  }
}
